...
**样张的外壳别手写。** 三列并排、scale 缩放、切屏、放大、底部确认表单——这部分每次都一样，手写一遍就多一次踩坑的机会。
...
`proof/` 里放 `proof.json`（轴、三个变体名、三屏名）和 `v1s1.html … v3s3.html`。
它会拦下两件事，拦不过就不出文件：**片段里有占位文字**，或者**不是正好 3 变体 × 3 屏**。
不想用脚本就照 `assets/preview-template.html` 手搭，但那两条规矩一样要守。
