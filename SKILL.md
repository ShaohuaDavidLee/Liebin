...
现成页面是拿文案和密度用的，**不要把它摆进确认页当「基线」列。**
用户自己有现在的站，会开；没有的人看一列「现在」只会乱。确认页只放三列：贴着做 / 取其神 / 反着来。
...
样张交出去之后，问三个问题，一个都不能省。确认页用选项，不要丢空文本框，也**不要把「现在的页面」摆上去当基线。**

第 2、3 问**固定用外壳里那一组**：每问 3 条 +「其他」。硬规矩：

- 不许按贴着做 / 取其神 / 反着来再拆一遍——那会变成十几条，人就不知道怎么选
- 不许把这个产品的具体剧情写成选项（「闭门会、二三十人扫码」那种话，让用户写在「其他」）
- 「其他」是一行输入，不是第二块作文题
- 点中的句子和「其他」里的字，逐字进 DESIGN.md，不要润色

1. **选哪个？** ——只能答「贴着做 / 取其神 / 反着来」，或者三个都不要。
2. **另外两个，哪里不要？** ——太满、太空、气质不对，三选多选。「那片绿刺眼」这种原话写在「其他」，并点名是哪一版。
3. **你的用户在什么状态下打开这个产品？** ——手机单手、电脑或大屏、赶时间。更具体的现场写在「其他」。如果三个变体全都答不上这一问，就该重做，而不是硬选一个。
...
- **把下面「实现时的创意指令」三段原文写进 DESIGN.md，不要改写。** 方向锁定之后，执行 agent 还要做大量未拍板的选择；这段话是给它的，不是给样张阶段的。
...
方向已经确认。下面三段**原文**进 DESIGN.md，一个字都不要润色。执行 agent 读到它们，才开始写页面。
...
- 确认之后：一份 **DESIGN.md**，每个数值标了出处，并附上实现时的创意指令原文
