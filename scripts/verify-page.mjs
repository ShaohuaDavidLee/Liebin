...
const consoleErrors = [];
const FONT_NOISE = /fonts\.googleapis\.com|fonts\.gstatic\.com/;
page.on("pageerror", (e) => consoleErrors.push("JS 异常：" + e.message));
page.on("console", (m) => {
  if (m.type() !== "error") return;
  const loc = m.location()?.url || "";
  if (FONT_NOISE.test(m.text() + loc)) return;
  consoleErrors.push("console.error：" + m.text().slice(0, 160));
});
...
for (const im of imgs) {
  ...
  if (!im.natW) { add("note", "图片没加载到，没法查形变和规格", name); continue; }
  if (!im.cw || !im.ch) continue;
  // object-fit 默认是 fill：正是「CSS 只写 width、HTML height 还在」会把图抻变形的那条路。
  // cover / contain / scale-down 保比例，none 不缩放——这三种不查形变，仍查超规格。
  const keepsRatio = im.fit === "cover" || im.fit === "contain" || im.fit === "none" || im.fit === "scale-down";
  if (!keepsRatio) {
